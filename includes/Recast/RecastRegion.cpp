	...
	rcIntArray& cont)
{
	...
	if (cont.size() > 1)
	{
		for (int i1 = 0; i1 < cont.size(); )
		{
			int ni = (i1 + 1) % cont.size();
			if (cont[i1] == cont[ni])
			{
				for (int j = i1; j < cont.size() - 1; ++j)
					cont[j] = cont[j + 1];
				cont.pop();
			}
			else
	...
	unsigned short* srcReg)
{
	...
	rcRegion* regions = static_cast<rcRegion*>(rcAlloc(sizeof(rcRegion)*nreg, RC_ALLOC_TEMP));
	if (!regions)
	{
	...
	for (int i = 0; i < nreg; ++i)
		new(&regions[i]) rcRegion(static_cast<unsigned short>(i));
	...
	for (int i = 0; i < nreg; ++i)
	{
		...
		while (stack.size())
		{
			...
			for (int j = 0; j < creg.connections.size(); ++j)
			{
				...
				rcRegion& region = regions[creg.connections[j]];
				if (region.visited)
					continue;
				if (region.id == 0 || (region.id & RC_BORDER_REG))
					continue;
				// Visit
				stack.push(region.id);
				region.visited = true;
			}
		}
...
bool rcBuildDistanceField(rcContext* ctx, rcCompactHeightfield& chf)
{
	...
	unsigned short* src = static_cast<unsigned short*>(rcAlloc(sizeof(unsigned short)*chf.spanCount, RC_ALLOC_TEMP));
	if (!src)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildDistanceField: Out of memory 'src' (%d).", chf.spanCount);
		return false;
	}
	unsigned short* dst = static_cast<unsigned short*>(rcAlloc(sizeof(unsigned short)*chf.spanCount, RC_ALLOC_TEMP));
	if (!dst)
	{
	...
	const int borderSize, const int minRegionArea, const int mergeRegionArea)
{
	...
	rcScopedDelete<unsigned short> srcReg = static_cast<unsigned short*>(rcAlloc(sizeof(unsigned short)*chf.spanCount, RC_ALLOC_TEMP));
	if (!srcReg)
	{
	...
	rcScopedDelete<rcSweepSpan> sweeps = static_cast<rcSweepSpan*>(rcAlloc(sizeof(rcSweepSpan)*nsweeps, RC_ALLOC_TEMP));
	if (!sweeps)
	{
