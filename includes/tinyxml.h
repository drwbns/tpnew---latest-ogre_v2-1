...
#ifdef TIXML_USE_STL
	/// Constructor.
	TiXmlText(const std::string& initValue) : TiXmlNode(TEXT)
	{
		SetValue(initValue);
		cdata = false;
	}
...
public:
	...
	*/
	virtual const char* Parse(const char* p, TiXmlParsingData* data = nullptr, TiXmlEncoding encoding = TIXML_DEFAULT_ENCODING) override;
