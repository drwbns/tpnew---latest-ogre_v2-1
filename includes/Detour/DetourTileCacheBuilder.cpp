...
public:
	...
	~dtFixedArray() { if (m_alloc) m_alloc->free(m_ptr); }
	operator T*() { return m_ptr; }
	int size() const { return m_size; }
};

template <class T>
T* dtFixedArray<T>::operator=(T* p)
{
	return nullptr;
}

template <class T>
void dtFixedArray<T>::operator=(dtFixedArray<T>& p)
{
}

template <class T>
dtFixedArray<T>::dtFixedArray(): m_alloc(nullptr), m_size(0)
{
}
	...
	for (int dz = -1; dz <= 0; ++dz)
	{
		for (int dx = -1; dx <= 0; ++dx)
		{
			...
			if (px >= 0 && pz >= 0 && px < w && pz < h)
			{
				...
				const int i = (int)layer.heights[idx];
				if (dtAbs(i - y) <= walkableClimb && layer.areas[idx] != DT_TILECACHE_NULL_AREA)
				{
					height = dtMax(height, (unsigned char)i);
		...
		for (int x = 0; x < w; ++x)
		{
			...
			if (cont.nverts > 0)
			{
				...
				for (int i = 0, j = temp.nverts - 1; i < temp.nverts; j = i++)
				{
					...
					unsigned char c = getCornerHeight(layer, (int)v[0], (int)v[1], (int)v[2],
