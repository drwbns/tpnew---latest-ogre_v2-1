...
void dtNavMesh::unconnectExtLinks(dtMeshTile* tile, dtMeshTile* target) const
{
...
void dtNavMesh::connectExtLinks(dtMeshTile* tile, dtMeshTile* target, int side) const
{
	if (!tile) return;
...
void dtNavMesh::connectExtOffMeshLinks(dtMeshTile* tile, dtMeshTile* target, int side) const
{
	if (!tile) return;
	...
	for (int i = 0; i < target->header->offMeshConCount; ++i)
	{
		...
		{
			unsigned int idx1 = allocLink(tile);
			if (idx1 != DT_NULL_LINK)
			{
				const unsigned short landPolyIdx = (unsigned short)decodePolyIdPoly(ref);
				dtPoly* landPoly = &tile->polys[landPolyIdx];
				dtLink* link = &tile->links[idx1];
...
void dtNavMesh::connectIntOffMeshLinks(dtMeshTile* tile) const
{
	if (!tile) return;
	...
	float* nearestPt) const
{
	...
	// Find nearest polygon amonGAMESTATE the nearby polygons.
	dtPolyRef nearest = 0;
...
boost::mutex adderMutex;

dtStatus dtNavMesh::addTile(unsigned char* data, int dataSize, int flags,
	dtTileRef lastRef, dtTileRef* result)
{
	boost::mutex::scoped_lock lock(adderMutex);
	// Make sure the data is in right format.
	dtMeshHeader* header = (dtMeshHeader*)data;
	if (header->magic != DT_NAVMESH_MAGIC)
		return DT_FAILURE | DT_WRONG_MAGIC;
	...
	tile->offMeshCons = (dtOffMeshConnection*)d;
...
const dtMeshTile* dtNavMesh::getTileByRef(dtTileRef ref) const
{
	if (!ref)
		return 0;
	unsigned int tileIndex = decodePolyIdTile(static_cast<dtPolyRef>(ref));
	unsigned int tileSalt = decodePolyIdSalt(static_cast<dtPolyRef>(ref));
...
dtStatus dtNavMesh::getTileAndPolyByRef(const dtPolyRef ref, const dtMeshTile** tile, const dtPoly** poly) const
{
	...
	if (ip >= static_cast<unsigned int>(m_tiles[it].header->polyCount)) return DT_FAILURE | DT_INVALID_PARAM;
	*tile = &m_tiles[it];
...
bool dtNavMesh::isValidPolyRef(dtPolyRef ref) const
{
	...
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
	if (it >= static_cast<unsigned int>(m_maxTiles)) return false;
	if (m_tiles[it].salt != salt || m_tiles[it].header == 0) return false;
	if (ip >= static_cast<unsigned int>(m_tiles[it].header->polyCount)) return false;
	return true;
}
...
dtTileRef dtNavMesh::getTileRef(const dtMeshTile* tile) const
{
	...
	return static_cast<dtTileRef>(encodePolyId(tile->salt, it, 0));
}
...
dtStatus dtNavMesh::storeTileState(const dtMeshTile* tile, unsigned char* data, const int maxDataSize) const
{
	...
	dtTileState* tileState = reinterpret_cast<dtTileState*>(data); data += dtAlign4(sizeof(dtTileState));
	dtPolyState* polyStates = reinterpret_cast<dtPolyState*>(data);
...
dtStatus dtNavMesh::restoreTileState(dtMeshTile* tile, const unsigned char* data, const int maxDataSize) const
{
	// Make sure there is enough space to store the state.
	const int sizeReq = getTileStateSize(tile);
	if (maxDataSize < sizeReq)
	...
	const dtTileState* tileState = reinterpret_cast<const dtTileState*>(data); data += dtAlign4(sizeof(dtTileState));
	const dtPolyState* polyStates = reinterpret_cast<const dtPolyState*>(data);
...
dtStatus dtNavMesh::setPolyFlags(dtPolyRef ref, unsigned short flags) const
{
	if (!ref) return DT_FAILURE;
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
...
dtStatus dtNavMesh::setPolyArea(dtPolyRef ref, unsigned char area) const
{
	if (!ref) return DT_FAILURE;
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
