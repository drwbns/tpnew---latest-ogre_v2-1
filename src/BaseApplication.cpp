...
BaseApplication::BaseApplication(void)
	...
	mMouse(nullptr),
	mKeyboard(nullptr), mChar(nullptr), mMode(0), lockedCam(0), yaw(0), pitch(0), moveScale(0), camToPlayer(0),
	mMouseSpeed(5),
	mRotateSpeed(50),
	mZoomSpeed(50), fixedCam(false), fixedYaw(false), fixedPitch(false), camRotate(false), yawHasBeenReset(false), 
	pitchAngle(0), yawAngle(0), 
	//guiDbg(nullptr), 
	mTimeUntilNextToggle(0), mFiltering(), mAniso(0), mSceneDetailIndex(0)
{
}
...
void BaseApplication::createCamera(void)
{
	...
	// mPosition it at 500 in Z direction
...
bool BaseApplication::frameRenderingQueued(const FrameEvent& evt)
{
	...
	*/
	...
	if (fixedPitch == false) {
		...
		mChar->getPitchNode()->pitch(static_cast<Radian>(pitchAngle) * angleSign, Node::TS_LOCAL);
	...
	*/
	if (mKeyboard->isKeyDown(OIS::KC_LCONTROL) && camDistToPlayer > 10) {
		...
		mChar->getYawNode()->yaw(static_cast<Radian>(yawAngle) * angleSign, Node::TS_LOCAL);
	}
