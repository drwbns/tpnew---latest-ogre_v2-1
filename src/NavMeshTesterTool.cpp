...
NavMeshTesterTool::NavMeshTesterTool() :
	...
	m_sposSet(false),
	m_eposSet(false),
	m_pathIterNum(0), m_pathIterPolyCount(0),
	m_steerPointCount(0)
{
...
void NavMeshTesterTool::handleToggle()
{
	...
	else if (offMeshConnection && inRange(m_iterPos, steerPos, SLOP, 1.0f))
	{
		...
		if (dtStatusSucceed(status))
		{
			...
			float x;
			m_navQuery->getPolyHeight(m_pathIterPolys[0], m_iterPos, &x);
			m_iterPos[1] = x;
		}
	}
...
void NavMeshTesterTool::handleUpdate()
{
				...
				while (npolys && m_nsmoothPath < MAX_SMOOTH)
				{
					...
					else if (offMeshConnection && inRange(iterPos, steerPos, SLOP, 1.0f))
					{
						...
						if (dtStatusSucceed(status))
						{
							...
							float x;
							m_navQuery->getPolyHeight(polys[0], iterPos, &x);
							iterPos[1] = x;
						}
					}
...
void NavMeshTesterTool::handleRender()
{
	...
	const float agentRadius = this->getAgentRadius();
	const float agentHeight = this->getAgentHeight();
	const float agentClimb = this->getAgentClimb();
	...
	if (m_toolMode == TOOLMODE_PATHFIND_FOLLOW)
	{
		duDebugDrawNavMeshPoly(mNavMeshDebugger, *m_navMesh, m_startRef, startCol);
		duDebugDrawNavMeshPoly(mNavMeshDebugger, *m_navMesh, m_endRef, endCol);

		if (m_npolys)
		{
			for (int i = 1; i < m_npolys - 1; ++i)
				duDebugDrawNavMeshPoly(mNavMeshDebugger, *m_navMesh, m_polys[i], pathCol);
		}

		if (m_nsmoothPath)
		{
			//dd.depthMask(false);
			const unsigned int path_col = duRGBA(0, 0, 0, 220);
			mNavMeshDebugger->begin("mNavMesherDebuggerMaterial", RenderOperation::OT_LINE_LIST);
			for (int i = 0; i < m_nsmoothPath; ++i)
				mNavMeshDebugger->position(m_smoothPath[i * 3], m_smoothPath[i * 3 + 1] + 0.1f, m_smoothPath[i * 3 + 2]);
		...
		if (m_pathIterNum)
		{
			duDebugDrawNavMeshPoly(mNavMeshDebugger, *m_navMesh, m_pathIterPolys[0], duRGBA(255, 255, 255, 128));
			...
			mNavMeshDebugger->position(m_prevIterPos[0], m_prevIterPos[1] - 0.3f, m_prevIterPos[2]);
			mNavMeshDebugger->position(m_prevIterPos[0], m_prevIterPos[1] + 0.3f, m_prevIterPos[2]);

			mNavMeshDebugger->position(m_iterPos[0], m_iterPos[1] - 0.3f, m_iterPos[2]);
			mNavMeshDebugger->position(m_iterPos[0], m_iterPos[1] + 0.3f, m_iterPos[2]);
			...
			mNavMeshDebugger->position(m_prevIterPos[0], m_prevIterPos[1] + 0.3f, m_prevIterPos[2]);
			mNavMeshDebugger->position(m_steerPos[0], m_steerPos[1] + 0.3f, m_steerPos[2]);

			for (int i = 0; i < m_steerPointCount - 1; ++i)
			{
				mNavMeshDebugger->position(m_steerPoints[i * 3 + 0], m_steerPoints[i * 3 + 1] + 0.2f, m_steerPoints[i * 3 + 2]);
				mNavMeshDebugger->position(m_steerPoints[(i + 1) * 3 + 0], m_steerPoints[(i + 1) * 3 + 1] + 0.2f, m_steerPoints[(i + 1) * 3 + 2]);
			}
	...
	else if (m_toolMode == TOOLMODE_PATHFIND_STRAIGHT || m_toolMode == TOOLMODE_PATHFIND_SLICED)
	{
		...
		{
			//dd.depthMask(false);
			const unsigned int path_col = duRGBA(64, 16, 0, 220);
			...
			for (int i = 0; i < m_nstraightPath - 1; ++i)
			{
				...
				mNavMeshDebugger->position(m_straightPath[i * 3], m_straightPath[i * 3 + 1] + 0.4f, m_straightPath[i * 3 + 2]);
				mNavMeshDebugger->position(m_straightPath[(i + 1) * 3], m_straightPath[(i + 1) * 3 + 1] + 0.4f, m_straightPath[(i + 1) * 3 + 2]);
			}
			mNavMeshDebugger->end();
			mNavMeshDebugger->begin("mNavMesherDebuggerMaterial", RenderOperation::OT_POINT_LIST);
	...
	{
		duDebugDrawNavMeshPoly(mNavMeshDebugger, *m_navMesh, m_startRef, pathCol);
		...
		if (m_nstraightPath)
		{
			...
			//dd.depthMask(false);
			const unsigned int path_col = m_hitResult ? duRGBA(64, 16, 0, 220) : duRGBA(240, 240, 240, 220);
			mNavMeshDebugger->begin("mNavMesherDebuggerMaterial", RenderOperation::OT_LINE_LIST);
			for (int i = 0; i < m_nstraightPath - 1; ++i)
			{
				mNavMeshDebugger->position(m_straightPath[i * 3], m_straightPath[i * 3 + 1] + 0.4f, m_straightPath[i * 3 + 2]);
			...
			if (m_hitResult)
			{
				...
				mNavMeshDebugger->begin("mNavMesherDebuggerMaterial", RenderOperation::OT_LINE_LIST);
				mNavMeshDebugger->position(m_hitPos[0], m_hitPos[1] + 0.4f, m_hitPos[2]);
				mNavMeshDebugger->position(m_hitPos[0] + m_hitNormal[0] * agentRadius,
					m_hitPos[1] + 0.4f + m_hitNormal[1] * agentRadius,
					m_hitPos[2] + m_hitNormal[2] * agentRadius);
				mNavMeshDebugger->end();
			}
			//dd.depthMask(true);
		}
	...
	else if (m_toolMode == TOOLMODE_DISTANCE_TO_WALL)
	{
		duDebugDrawNavMeshPoly(mNavMeshDebugger, *m_navMesh, m_startRef, startCol);
		//dd.depthMask(false);
		duDebugDrawCircle(mNavMeshDebugger, m_spos[0], m_spos[1] + agentHeight / 2, m_spos[2], m_distanceToWall, duRGBA(64, 16, 0, 220), 2.0f);
		mNavMeshDebugger->begin("mNavMesherDebuggerMaterial", RenderOperation::OT_LINE_LIST);
		mNavMeshDebugger->position(m_hitPos[0], m_hitPos[1] + 0.02f, m_hitPos[2]);
		mNavMeshDebugger->position(m_hitPos[0], m_hitPos[1] + agentHeight, m_hitPos[2]);
		mNavMeshDebugger->end();
		//dd.depthMask(true);
	}
	else if (m_toolMode == TOOLMODE_FIND_POLYS_IN_CIRCLE)
	{
		for (int i = 0; i < m_npolys; ++i)
		{
			...
			if (m_parent[i])
			{
				...
				duDebugDrawArc(mNavMeshDebugger, p0[0], p0[1], p0[2], p1[0], p1[1], p1[2], 0.25f, 0.0f, 0.4f, duRGBA(0, 0, 0, 128), 2.0f);
				//dd.depthMask(true);
			}
			//dd.depthMask(true);
		}
		...
		if (m_sposSet && m_eposSet)
		{
			...
			duDebugDrawCircle(mNavMeshDebugger, m_spos[0], m_spos[1] + agentHeight / 2, m_spos[2], dist, duRGBA(64, 16, 0, 220), 2.0f);
			//dd.depthMask(true);
		}
	}
	else if (m_toolMode == TOOLMODE_FIND_POLYS_IN_SHAPE)
	{
		...
		if (m_sposSet && m_eposSet)
		{
			...
			for (int i = 0, j = 3; i < 4; j = i++)
			{
				...
				//mNavMeshDebugger->position(p0, );
				//mNavMeshDebugger->position(p1, );
			}
			mNavMeshDebugger->end();
			//dd.depthMask(true);
		}
	...
	else if (m_toolMode == TOOLMODE_FIND_LOCAL_NEIGHBOURHOOD)
	{
		for (int i = 0; i < m_npolys; ++i)
		{
			...
			for (int j = 0; j < nsegs; ++j)
			{
				...
				if (refs[j])
				{
					...
					mNavMeshDebugger->position(s[0], s[1] + agentClimb, s[2]);
					mNavMeshDebugger->position(s[3], s[4] + agentClimb, s[5]);
				}
				else
				{
					...
					mNavMeshDebugger->position(p0[0], p0[1] + agentClimb, p0[2]);
					mNavMeshDebugger->position(p1[0], p1[1] + agentClimb, p1[2]);
		...
		{
			//dd.depthMask(false);
			duDebugDrawCircle(mNavMeshDebugger, m_spos[0], m_spos[1] + agentHeight / 2, m_spos[2], m_neighbourhoodRadius, duRGBA(64, 16, 0, 220), 2.0f);
			//dd.depthMask(true);
		}
	}
}
...
void NavMeshTesterTool::drawAgent(const float* pos, float r, float h, float c, const unsigned int col)
{
	...
	// Agent dimensions.
	duDebugDrawCylinderWire(mNavMeshDebugger, pos[0] - r, pos[1] + 0.02f, pos[2] - r, pos[0] + r, pos[1] + h, pos[2] + r, col, 2.0f);

	duDebugDrawCircle(mNavMeshDebugger, pos[0], pos[1] + c, pos[2], r, duRGBA(0, 0, 0, 64), 1.0f);

	unsigned int colb = duRGBA(0, 0, 0, 196);
	mNavMeshDebugger->begin("mNavMesherDebuggerMaterial", RenderOperation::OT_LINE_LIST);
	mNavMeshDebugger->position(pos[0], pos[1] - c, pos[2]);
	mNavMeshDebugger->position(pos[0], pos[1] + c, pos[2]);
	mNavMeshDebugger->position(pos[0] - r / 2, pos[1] + 0.02f, pos[2]);
	mNavMeshDebugger->position(pos[0] + r / 2, pos[1] + 0.02f, pos[2]);
	mNavMeshDebugger->position(pos[0], pos[1] + 0.02f, pos[2] - r / 2);
	mNavMeshDebugger->position(pos[0], pos[1] + 0.02f, pos[2] + r / 2);
	mNavMeshDebugger->end();
