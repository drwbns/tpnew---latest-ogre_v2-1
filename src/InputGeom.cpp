...
template<> SharedData* Singleton<SharedData>::msSingleton = nullptr;

template<> InputGeom* Singleton<InputGeom>::msSingleton = nullptr;
...
bool InputGeom::save(const char* filepath)
{
	...
	for (int i = 0; i < m_volumeCount; ++i)
	{
		...
		for (int i1 = 0; i1 < vol->nverts; ++i1)
			fprintf(fp, "%f %f %f\n", vol->verts[i1 * 3 + 0], vol->verts[i1 * 3 + 1], vol->verts[i1 * 3 + 2]);
	}
...
bool InputGeom::raycastMesh(float* src, float* dst, float& tmin) const
{
	...
	const float* verts = &m_mesh->getVerts()[0];
	const float* normals = m_mesh->getNormals();
	const size_t* tris = &m_mesh->getTris()[0];
	tmin = 1.0f;
	bool hit = false;
