...
DotSceneLoader::DotSceneLoader() : mSceneMgr(nullptr), mAttachNode(nullptr), mTerrainGroup(nullptr)
{
...
void DotSceneLoader::processTerrainPage(rapidxml::xml_node<>* XMLNode)
{
	...
	else
	{
		...
		while (pElement)
		{
			...
			while (pTerrainLayer)
			{
				int world_size = Ogre::StringConverter::parseInt(pTerrainLayer->first_attribute("worldSize")->value());
				...
				while (pTerrainTexture)
				{
					...
					imp.layerList[count].worldSize = static_cast<Ogre::Real>(world_size);
...
void DotSceneLoader::processBlendmaps(rapidxml::xml_node<>* XMLNode) const
{
	int pageX = Ogre::StringConverter::parseInt(XMLNode->first_attribute("pageX")->value());
	int pageY = Ogre::StringConverter::parseInt(XMLNode->first_attribute("pageY")->value());
...
void DotSceneLoader::processLookTarget(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent) const
{
...
void DotSceneLoader::processEntity(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
	...
	//      ;//processIndexBuffer(pElement);
	  ...
	  // Create the entity
	Ogre::v1::Entity *pEntity = nullptr;
	try
	{
		Ogre::MeshPtr mymesh = Ogre::MeshManager::getSingleton().load(meshFile, m_sGroupName);

		mymesh.get()->setLodStrategy(Ogre::PixelCountLodStrategy::getSingletonPtr()); // might need to reapply after load
...
void DotSceneLoader::processParticleSystem(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent) const
{
	// Process attributes
	Ogre::String name = getAttrib(XMLNode, "name");
	Ogre::String id = getAttrib(XMLNode, "id");
	...
	try
	{
		...
		pParent->attachObject(reinterpret_cast<Ogre::MovableObject*>(pParticles));
	}
	catch (Ogre::Exception &/*e*/)
	{
...
void DotSceneLoader::processBillboardSet()
{
	//! @todo Implement this
}

void DotSceneLoader::processPlane()
{
	//! @todo Implement this
}
...
void DotSceneLoader::processUserDataReference(rapidxml::xml_node<>* XMLNode, Ogre::v1::Entity *pEntity)
{
