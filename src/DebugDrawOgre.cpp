	...
	const float walkableSlopeAngle, const float texScale)
{
	...
	dd->begin("mNavMesherDebuggerMaterial", Ogre::v1::RenderOperation::OT_LINE_LIST);
	for (int i = 0; i < ntris * 3; i += 3)
	{
		const float* norm = &normals[i];
		unsigned int color;
		unsigned char a = static_cast<unsigned char>(220 * (2 + norm[0] + norm[1]) / 4);
		...
		dd->position(static_cast<Ogre::Real>(*va), static_cast<Ogre::Real>(*vb), static_cast<Ogre::Real>(*vc));
	}
	dd->end();
...
void DebugDrawOgre::duDebugDrawArrow()
{
}
...
void DebugDrawOgre::drawConvexVolumes(Ogre::ManualObject* dd, bool /*hilight*/) const
{
	//dd->depthMask(false);
	...
	dd->begin("mNavMesherDebuggerMaterial", Ogre::v1::RenderOperation::OT_POINT_LIST);
	for (int i = 0; i < m_volumeCount; ++i)
	{
		const ConvexVolume* vol = &m_volumes[i];
...
void DebugDrawOgre::duAppendCylinder()
{
}
...
void DebugDrawOgre::duDebugDrawCross()
{
}

void DebugDrawOgre::duDebugDrawBox()
{
}

void DebugDrawOgre::duDebugDrawCylinder()
{
}
...
void DebugDrawOgre::duAppendBoxPoints()
{
}
...
void DebugDrawOgre::drawPolyBoundaries(Ogre::ManualObject* dd, const dtMeshTile* tile, const float linew,
                                       bool inner)
{
	...
	const dtMeshTile* tile, unsigned char flags)
{
	...
	dd->begin("mNavMesherDebuggerMaterial", Ogre::v1::RenderOperation::OT_TRIANGLE_LIST);
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		const dtPoly* p = &tile->polys[i];
		...
		for (int j = 0; j < pd->triCount; ++j)
		{
			...
			{
				if (t[k] < p->vertCount)
					dd->position(static_cast<Real>tile->verts[p->verts[t[k]]*3],static_cast<Real>tile->verts[p->verts[t[k+1]]*3],static_cast<Real>tile->verts[p->verts[t[k+2]]*3]);
				else
					dd->position(static_cast<Real>tile->detailVerts[(pd->vertBase+t[k]-p->vertCount)*3],static_cast<Real>tile->detailVerts[(pd->vertBase+t[k+1]-p->vertCount)*3],static_cast<Real>tile->detailVerts[(pd->vertBase+t[k+2]-p->vertCount)*3]);
			}
			*/
		}
	}
	...
	if (flags & DU_DRAWNAVMESH_OFFMESHCONS)
	{
		...
		for (int i = 0; i < tile->header->polyCount; ++i)
		{
			...
			if (startSet)
			{
				dd->position(static_cast<Real>(va[0]), static_cast<Real>(va[1]), static_cast<Real>(va[2]));
				dd->position(static_cast<Real>(con->pos[0]), static_cast<Real>(con->pos[1]), static_cast<Real>(con->pos[2]));
				duAppendCircle(dd, con->pos[0], con->pos[1] + 0.1f, con->pos[2], con->rad, duRGBA(0, 48, 64, 196));
			}
			if (endSet)
			{
				dd->position(static_cast<Real>(vb[0]), static_cast<Real>(vb[1]), static_cast<Real>(vb[2]));
				dd->position(static_cast<Real>(con->pos[3]), static_cast<Real>(con->pos[4]), static_cast<Real>(con->pos[5]));
...
void DebugDrawOgre::duDebugDrawNavMesh(Ogre::ManualObject* dd, const dtNavMesh& mesh, unsigned char flags)
{
	...
	for (int i = 0; i < mesh.getMaxTiles(); ++i)
	{
		...
		drawMeshTile(dd, mesh, nullptr, tile, flags);
	}
}
...
void DebugDrawOgre::duDebugDrawTileCacheLayer(Ogre::ManualObject* dd, const dtTileCacheLayer& layer, const float cs, const float ch) const
{
	const int w = static_cast<int>(layer.header->width);
	const int h = static_cast<int>(layer.header->height);
	...
	duDebugDrawBoxWire(dd, lbmin[0], lbmin[1], lbmin[2], lbmax[0], lbmax[1], lbmax[2], duTransCol(color, 128));
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			const int idx1 = x + y * w;
			const int i = static_cast<int>(layer.heights[idx1]);
			if (i == 0xff) continue;
			const unsigned char area = layer.areas[idx1];
...
void DebugDrawOgre::duDebugDrawTriMesh()
{
}
...
void DebugDrawOgre::duDebugDrawHeightfieldSolid(Ogre::ManualObject* dd, const rcHeightfield& hf)
{
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			while (s)
			{
				duAppendBox(dd, fx, orig[1] + s->smin * ch, fz, fx + cs, orig[1] + s->smax * ch, fz + cs);
				s = s->next;
			}
		}
	}
...
void DebugDrawOgre::duDebugDrawHeightfieldWalkable(Ogre::ManualObject* dd, const rcHeightfield& hf) const
{
	if (!dd) return;
	...
	for (int y = 0; y < chf.height; ++y)
	{
		for (int x = 0; x < chf.width; ++x)
		{
			...
			for (unsigned i = c.index, ni = c.index + c.count; i < ni; ++i)
			{
				const rcCompactSpan& s = chf.spans[i];
				const float fy = chf.bmin[1] + (s.y + 1) * ch;
				const unsigned char cd = static_cast<unsigned char>(chf.dist[i] * dscale);
...
void DebugDrawOgre::drawLayerPortals(Ogre::ManualObject* dd, const rcHeightfieldLayer* layer)
{
	...
	for (int y = 0; y < h; ++y)
	{
		...
		{
			const int idx = x + y * w;
			const int i = static_cast<int>(layer->heights[idx]);
			if (i == 255) continue;
...
void DebugDrawOgre::duDebugDrawHeightfieldLayer(Ogre::ManualObject* dd, const struct rcHeightfieldLayer& layer, const int idx) const
{
	const float cs = layer.cs;
	const float ch = layer.ch;
	const int w = layer.width;
	...
	duDebugDrawBoxWire(dd, bmin[0], bmin[1], bmin[2], bmax[0], bmax[1], bmax[2], duTransCol(color, 128));
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			const float fy = layer.bmin[1] + (i + 1) * ch;
...
void DebugDrawOgre::duDebugDrawHeightfieldLayers(Ogre::ManualObject* dd, const struct rcHeightfieldLayerSet& lset) const
{
	if (!dd) return;
...
void DebugDrawOgre::duDebugDrawHeightfieldLayersRegions(ManualObject* dd)
{
}
...
void DebugDrawOgre::duDebugDrawRegionConnections(Ogre::ManualObject* dd, const rcContourSet& cset, const float alpha) const
{
	if (!dd) return;
	...
	for (int i = 0; i < cset.nconts; ++i)
	{
		...
		for (int j = 0; j < cont->nverts; ++j)
		{
			...
			if (v[3] == 0 || static_cast<unsigned short>(v[3]) < cont->reg) continue;
			const rcContour* cont2 = findContourFromSet(cset, static_cast<unsigned short>(v[3]));
			if (cont2)
			{
	...
	{
		const rcContour* cont = &cset.conts[i];
		unsigned int color1 = duDarkenCol(duIntToCol(cont->reg, a));
		getContourCenter(cont, orig, cs, ch, pos);
...
void DebugDrawOgre::duDebugDrawRawContours(Ogre::ManualObject* dd, const rcContourSet& cset, const float alpha) const
{
	if (!dd) return;
...
void DebugDrawOgre::duDebugDrawContours(Ogre::ManualObject* dd, const rcContourSet& cset, const float alpha) const
{
	if (!dd) return;
...
void DebugDrawOgre::duDebugDrawPolyMeshDetail(Ogre::ManualObject* dd, const struct rcPolyMeshDetail& dmesh) const
{
	if (!dd) return;
